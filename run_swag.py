...
def main():
    ...
    def preprocess_function(examples):
        ...
        second_sentences = [[context[pid] for pid in paragraph_ids] for paragraph_ids in examples['paragraphs']]

        if not training_args.do_predict:
            relevants = examples["relevant"]
            labels = [paragraph_ids.index(relevants[i]) for i, paragraph_ids in enumerate(examples['paragraphs'])]
    ...
    if training_args.do_predict:
        ...
        if data_args.max_eval_samples is not None:
            pred_dataset = pred_dataset.select(range(data_args.max_eval_samples))
    ...
    if training_args.do_predict:
        ...
        import csv
        with open('prediction.choice.csv', 'w') as f:
            w = csv.writer(f)
            w.writerows(predictions)
        # max_eval_samples = data_args.max_eval_samples if data_args.max_eval_samples is not None else len(pred_dataset)
        # metrics["eval_samples"] = min(max_eval_samples, len(pred_dataset))

        # trainer.log_metrics("pred", metrics)
        # trainer.save_metrics("pred", metrics)
